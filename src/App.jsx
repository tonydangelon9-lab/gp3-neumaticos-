...
// ─── PRODUCTOS FIJOS — 9 modelos hardcodeados ─────────────────────────────────
const PRODUCTOS = [
...
export default function App(){
 ...
 const tabs=isAdmin?[["venta","🛒 Venta"],["stock","📦 Stock"],["estadisticas","📊 Stats"],["cierre","🗂 Cierre"],["gestion","⚙️ Gestión"]]:[["venta","🛒 Venta"],["mis_stats","📊 Mi Resumen"]];
       ...
       {tab==="stock"&&isAdmin&&(
         <Card><CardHeader>Control de Stock Pirelli</CardHeader>
           <div style={{padding:12}}>
             ...
             {!stockDraft?(<Btn onClick={()=>setStockDraft({...stock})} outline style={{marginBottom:12}}>✏️ Editar Stock</Btn>):(<div style={{display:"flex",gap:8,marginBottom:12}}><Btn color={C.green} onClick={()=>{setStock(stockDraft);syncSheets("stock",{stock:stockDraft,nombres:Object.fromEntries(todosLosProductos.map(p=>[p.id,{label:p.label,tipo:p.tipo}]))});setStockDraft(null);boom("✓ Stock guardado");}}>💾 Guardar</Btn><Btn outline onClick={()=>setStockDraft(null)}>Cancelar</Btn></div>)}
             <div style={{overflowX:"auto"}}>
               ...
               {todosLosProductos.map(p=>{
                 ...
                 return(<div key={p.id} style={{display:"grid",gridTemplateColumns:"1fr 90px 90px 90px 60px 1fr",padding:"14px 10px",borderBottom:`1px solid ${C.border}`,gap:8,alignItems:"center",minWidth:520}}>
                   <div><div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:15}}>{p.label}</div><div style={{fontSize:11,color:C.gray}}>USD {precios[p.id]?.USD??p.precios.USD} / ARS {(precios[p.id]?.ARS??p.precios.ARS).toLocaleString()}</div></div>
